/// The `country` value for a lookup request.
///
/// `StoreKit.Storefront.current.countryCode` reports ISO 3166-1
/// **alpha-3** ("JPN"). The lookup API's `country` parameter takes **alpha-2**
/// ("jp"). Handing the alpha-3 straight through does not fail — the API ignores
/// what it cannot parse and serves the **US** storefront, so names, prices,
/// ratings and availability are all silently wrong for every non-US user.
///
/// ```swift
/// let code = StorefrontCode(alpha3: await Storefront.current?.countryCode ?? "USA")
/// ```
public struct StorefrontCode: Sendable, Equatable, Hashable {
    /// Ready to interpolate into `&country=`.
    public let parameterValue: String

    /// Takes the ISO 3166-1 alpha-3 code `StoreKit.Storefront` reports.
    ///
    /// An unrecognized code falls back to `us` rather than propagating: a real
    /// page in a real language beats a silently wrong storefront, and Apple
    /// opens storefronts on a schedule this table does not follow.
    public init(alpha3: String) {
        parameterValue = Self.alpha2[alpha3.uppercased()] ?? Self.fallback
    }

    /// The storefront served when the alpha-3 is unrecognized.
    public static let fallback = "us"

    /// Guards against the table being trimmed — a dropped row degrades silently
    /// to `us` for everyone in that storefront. Internal: this exists for the
    /// test that watches the table's size, not for callers.
    static var knownAlpha3Count: Int { alpha2.count }

    /// ISO 3166-1, generated from the published register rather than typed by
    /// hand. Regenerate rather than edit in place.
